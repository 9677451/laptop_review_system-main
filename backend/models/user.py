...
class UserModel:
    ...
    def get_user_by_id(user_id):
        query = "SELECT user_id, username, email, phone, occupation, role, points, level FROM users WHERE user_id = ?"
    ...
    def add_points(user_id, points):
        ...
        db.execute_update("UPDATE users SET points = points + ? WHERE user_id = ?", (points, user_id))
        db.execute_update("UPDATE users SET level = FLOOR(points / 100) + 1 WHERE user_id = ?", (user_id,))
        return True
